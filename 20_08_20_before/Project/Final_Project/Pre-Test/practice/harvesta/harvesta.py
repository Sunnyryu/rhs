    ...
    for keywordSource in keyword_source_dic.keys():
        ...
        for i, keywords in enumerate(keywordsList):
            if _doNaver:
                ...
                naverList = naverfuncs.get_newslist(keywords, 30, datebf7, date)
                util.insert_dfrow(
                    df, keywordSource, keywords, i, 'NAVER', naverList)
            if _doDaum:
                print("{} 검색어 {} DAUM 뉴스 수집 시작... ".format(
                    keywordSource, i+1), flush=True)
                daumList =  daumfuncs.get_newslist(keywords, 30, datebf7, date)
                util.insert_dfrow(
            ...
            if _doGoogle:
                ...
                if doGoogling :
                    print("{} 검색어 {} Google 뉴스 수집 시작... ".format(
                        keywordSource, i+1), flush=True)
                    googleList = googlefuncs.get_newslist(keywords, 30,datebf7,date)
                    if googleList != -1:
