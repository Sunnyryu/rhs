...
def harvest(startpath, datebf7, date, keyword_source_dic):
# 시작 시간
    stime = time.time()
            ...
            if _doGoogle:
                ...
                if doGoogling :
                    print("{} 검색어 {} Google 뉴스 수집 시작... ".format(
                        keywordSource, i+1), flush=True)
                    googleList = googlefuncs.get_newslist(keywords, 12, datebf7 ,date)
                    if googleList != -1:
    ...
    if now.month >= 10:
        if now.hour >= 10:
            ...
            outputRoot = os.path.join(
                ...
                "%d%d%02d" % (now.year % 100, now.month, now.day),
                # "%02d%02d" % (now.hour, now.minute // 10 * 10)
                f'%02d{now.minute}' % f'%02d{now.hour}'
            )
        ...
        if now.hour >= 10:
            outputRoot = os.path.join(
                startpath,
                "%d%02d%02d" % (now.year % 100, now.month, now.day),
                # "%02d%02d" % (now.hour, now.minute // 10 * 10)
                f'%02d{now.minute}' % (now.hour)
            )
        else:
    ...
    # 릴리즈 시 주석처리
    #--------------------------------------------------------------------------
    csvPathName = os.path.join(outputRoot, f'output.csv')
    print(now.minute)
    csvTwPathName = os.path.join(outputRoot, "output_tw.csv")
    df.to_csv(csvPathName, mode='a', index=False, encoding="utf-8")
    ...
    initialDic = {'NAVER':'N', 'DAUM':'D', 'Google':'G','ENTERPRISE':'E','HOBBY':'H'}
    # # 로그 파일
    ...
    dfGoogle = df[df.nsource == 'Google'][:]
    toLen = max(len(dfNaver.index), len(dfDaum.index),len(dfGoogle.index))
    print(toLen)
    for i in range(toLen):
        print(len(dfNaver.index), len(dfDaum.index),len(dfGoogle.index),toLen)
        ...
        if i < len(dfGoogle.index):
            dfShuffle.loc[len(dfShuffle.index)] = list(dfGoogle.iloc[i])
    df = dfShuffle
    del(dfShuffle)
    ...
    for i in range(count):
        ...
        # ENTERPRISE, HOBBY 번갈아 다운로드 하기 때문에 싸이트별로는 3초 쉬는 효과
        time.sleep(1.5)
...
if __name__ == "__main__":
    ...
    from datetime import datetime
    import pandas as pd
    harvest("../../../data/timeline/191029/1400","20190210","20190211",{'ENTERPRISE':["삼성중공업"]})
