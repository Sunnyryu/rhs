...
def joined(json):
    ...
    k = 'history'
    emit('hello daebakbot', robot_dict, room = room)
...
@socketio.on('customer', namespace='/chat')
def customer_event(json, methods=['GET', 'POST']):
    ...
    if json['message']!='':
        ...
        if c_table_check == 1:
            ...
            c_table_insert = chat_Database.insert_message(c_number, c_name, c_message, c_time, c_YMH, table_name)
        emit('customer send', json, room=room)
    else:
        print('messeage is bin')



@socketio.on('robot event', namespace='/chat')
def robot_event(json, methods=['GET', 'POST']):
    ...
    if len(test_1) > 1:
        ...
        robot_dict['message'] = r_message
        emit('robot resend', robot_dict, room=room)
    try:
        if bot_content:
            robot_dict = {}
            robot_dict['name']= r_name
            ...
            print(robot_dict['link'], robot_dict['image'], 5959)
            emit('robot resend', robot_dict, room=room)
        else:
            print("길이가 1보다 작음")
            robot_dict = {}
